...
LSM303D_Acc::init()
{
    _device._extA.setCallback([this](uint32_t channel) {
            ...
            if (_runner != nullptr) {
                _timestamp.now();
                core::os::Thread::wake(*(this->_runner), LSM303D_ACC_DATA_READY);
                _runner = nullptr;
            }
...
LSM303D_Mag::init()
{
    _device._extM.setCallback([this](uint32_t channel) {
            ...
            if (_runner != nullptr) {
                _timestamp.now();
                core::os::Thread::wake(*(this->_runner), LSM303D_MAG_DATA_READY);
                _runner = nullptr;
            }
